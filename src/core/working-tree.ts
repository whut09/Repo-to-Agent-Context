...
function hashFileTree(root: string): string {
  ...
  return hashText(files.sort().map((file) => `${file}\0${hashText(readFileSync(path.join(root, file), "utf8"))}`).join("\n"));
}
...
function walk(root: string, directory: string, files: string[]): void {
  for (const item of readdirSync(directory, { withFileTypes: true }).sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    else if (item.isFile() && existsSync(absolute) && statSync(absolute).size <= 16 * 1024 * 1024) files.push(path.relative(root, absolute).replaceAll("\\", "/"));
  }
}
