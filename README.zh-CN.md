...
本文件保留为中文入口。完整产品说明请阅读 [README.md](README.md)。当前产品重点是 **Windows 端官方 OpenCode Desktop 插件**：双击 EXE 安装后，在 OpenCode Desktop 中提供 Guard 和证据能力；`/plusplus-task`、`/plusplus-verify` 和 `opencode-plusplus` skill 让会话直接走 Harness 工作流，`/opencode-plusplus-status`、`/opencode-plusplus-on` 和 `/opencode-plusplus-off` 由窄范围宿主补丁直接本地执行，不调用模型。
