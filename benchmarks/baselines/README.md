...
Compare a run with a baseline by passing `--baseline <file>`. Use `--fail-on-regression` when a scheduled or manual workflow should fail after any metric exceeds the configured absolute mean threshold.
