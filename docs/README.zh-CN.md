...
OpenCode++ 当前以 Windows 为重点：使用 Release EXE 把插件安装到官方 OpenCode Desktop，然后在聊天界面中查看状态、启用和关闭。CLI 与 MCP 是高级自动化入口，不是第二个 Desktop 应用。
...
| 使用 CLI                              | [CLI 参考](reference/cli-reference.zh-CN.md)                            |
