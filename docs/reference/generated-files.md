...
The Windows build produces three release assets under `release/`: the EXE, its SHA256 file, and `opencode-plusplus-release.json`. The manifest records the `package.json` version, platform, architecture, size budget, digest, plugin bundle sizes, the `opencode-plusplus` mode, and the legacy cleanup contract. These files are uploaded to a GitHub Release; they are ignored by Git and excluded from the npm developer package.
